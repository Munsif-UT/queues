...
async function sandbox(SandboxedJob) {
    ...
    let report_document = await sellingPartner.callAPI({
        ...
        path: {
            reportDocumentId: SandboxedJob.data.Report.reportDocumentId // retrieve the reportDocumentId from a "getReport" operation (when processingStatus of report is "DONE")
        }
    });
    ...
    let marketplacesData = await Marketplaces.find({ marketplaceId: { $in: SandboxedJob.data?.marketplaces } })
